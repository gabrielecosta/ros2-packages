...
class joyStickUtils():
    ...
    def extract_vel_commands(self, dict_data):
        x_l = dict_data['stick_sinistro']['X']
        y_l = dict_data['stick_sinistro']['Y']
        pulsante_lb = dict_data['pulsanti']['LB']
        pulsante_rb = dict_data['pulsanti']['RB']
        trigger_sinistro = dict_data['trigger']['sinistro']
        trigger_destro = dict_data['trigger']['destro']
        theta = 0
        if pulsante_lb == 1 and pulsante_rb == 0:
            theta = -1
        if pulsante_lb == 0 and pulsante_rb == 1:
            theta = 1
        if pulsante_rb == pulsante_lb:
            theta = 0
        omega = theta
        return x_l, y_l, omega, trigger_sinistro, trigger_destro